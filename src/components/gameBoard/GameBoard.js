...
const GameBoard = observer(class GameBoard extends Component {
	...
	onTimerEnd() {
		...
		axios.post('http://localhost:4000/analyzeImage', {
			...
			.then((response) => {
				if (response.data.highscore !== -1 && response.data.highscore) {
					console.log(response.data)
					let user = prompt("Please enter your name", "Anonymous");
					axios.post('http://localhost:4000/updateScore', {
						image: dataURL,
						index: response.data.highscore,
						score: Math.round(Math.pow(response.data.score, 6) * 100 * 100) / 100,
						name: user
					}).then((res) => {
